 ...
 var game = {
	...
	startState: function () {
		...
		//the values of and then rewrite them when startState is called
		 ...
		 //loops through character objects and resets their start position, 
		 //resets their display and opacity values, and removes 
		 //classes added during the game
		$('.character').each(function (i, val) {
			$(this).appendTo('.playerSelect').css({
	...
	attack: function () {
		...
		var heroCalc = game.hero.HP -= game.enemy.counterAP;
		var enemyCalc = game.enemy.HP -= game.hero.AP;
